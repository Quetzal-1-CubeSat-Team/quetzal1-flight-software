...
#ifdef ESP8266
  adcs_wire.setClockStretchLimit(1000); // Allow for 1000us of clock stretching
#endif
...
/**************************************************************************/
/*
  @brief  Sends a dummy command to test I2C communication.
  @return bool, true if no errors.
*/
/**************************************************************************/
bool Adafruit_BNO055::testComm(void)
{
  uint8_t ans;
  adcs_wire.beginTransmission(_address);
...
/**************************************************************************/
/*
  @brief The power mode can be selected by writing to the PWR_MODE register (0x3F).
  Suspended Mode (xxxxxx10b): In suspend mode the system is paused and all the
  sensors and the microcontroller are put into sleep mode. No values in the
  register map will be updated in this mode. To exit from suspend mode the mode
  should be changed by writing to the PWR_MODE register
*/
/**************************************************************************/
void Adafruit_BNO055::enterSuspendMode(void)
{
...
/**************************************************************************/
/*
  The power mode can be selected by writing to the PWR_MODE register (0x3F).
  Normal Mode (xxxxxx00b): In normal mode all sensors required for the selected
  operating mode (see section 3.3) are always switched ON. The register map and
  the internal peripherals of the MCU are always operative in this mode.
*/
void Adafruit_BNO055::enterNormalMode(void)
{
    adafruit_bno055_opmode_t modeback = _mode;
...
*/
/**************************************************************************/
void Adafruit_BNO055::setI2C(SoftwareWire i2c_bus)
{
  adcs_wire = i2c_bus;
}
...
bool Adafruit_BNO055::write8(adafruit_bno055_reg_t reg, byte value)
{
  adcs_wire.beginTransmission(_address);
  #if ARDUINO >= 100
    adcs_wire.write((uint8_t)reg);
    adcs_wire.write((uint8_t)value);
  #else
    adcs_wire.send(reg);
    adcs_wire.send(value);
  #endif
...
byte Adafruit_BNO055::read8(adafruit_bno055_reg_t reg )
{
  ...
  #endif
  adcs_wire.endTransmission();
  adcs_wire.requestFrom(_address, (byte)1);
  #if ARDUINO >= 100
    value = adcs_wire.read();
  #else
    value = adcs_wire.receive();
  #endif
...
bool Adafruit_BNO055::readLen(adafruit_bno055_reg_t reg, byte * buffer, uint8_t len)
{
  ...
  #endif
  adcs_wire.endTransmission();
  adcs_wire.requestFrom(_address, (byte)len);

  for (uint8_t i = 0; i < len; i++)
  {
    #if ARDUINO >= 100
      buffer[i] = adcs_wire.read();
    #else
      buffer[i] = adcs_wire.receive();
    #endif
  }

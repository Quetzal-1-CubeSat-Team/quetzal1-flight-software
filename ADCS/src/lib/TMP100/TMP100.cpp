...
{
  adcs_wire.beginTransmission(TMP100_i2caddr);
  adcs_wire.write(pointer);
...
bool TMP100::i2cReadBytes(uint8_t pointer, int16_t *value, uint8_t count)
{
  ...
  adcs_wire.beginTransmission(TMP100_i2caddr);
  adcs_wire.write(pointer);
  transmission = adcs_wire.endTransmission();

  reception = adcs_wire.requestFrom(TMP100_i2caddr, count);
  while ((adcs_wire.available() < count) && timeout--)
  {
    delay(1);  
  }
...
{ 
  adcs_wire.beginTransmission(TMP100_i2caddr); // Connect to TMP100
  adcs_wire.write(pointerReg);                 // Open specified register
  adcs_wire.endTransmission();                 // Close communication with TMP100
}
...
uint8_t TMP100::readRegister(bool registerNumber){  
  ...
  adcs_wire.requestFrom(TMP100_i2caddr, (uint8_t) 2);   // Read two bytes from TMP100
  registerByte[0] = (adcs_wire.read());       // Read first byte
  registerByte[1] = (adcs_wire.read());       // Read second byte
...
bool TMP100::wakeup(void)
{
  ...
  adcs_wire.beginTransmission(TMP100_i2caddr);
  adcs_wire.write(TMP100_CONFIG_REGISTER);                     // Point to configuration register
  adcs_wire.write(registerByte);                               // Write first byte
  int transmission = adcs_wire.endTransmission();              // Close communication with TMP100
